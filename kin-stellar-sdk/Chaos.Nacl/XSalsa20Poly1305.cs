...
namespace Kin.Stellar.Sdk.Chaos.Nacl
{
    public static class XSalsa20Poly1305
    {
        ...
        public static void Encrypt(ArraySegment<byte> ciphertext, ArraySegment<byte> message, ArraySegment<byte> key,
            ArraySegment<byte> nonce)
        {
            ...
            EncryptInternal(ciphertext.Array, ciphertext.Offset, message.Array, message.Offset, message.Count,
                key.Array, key.Offset, nonce.Array, nonce.Offset);
        }
        ...
        /// </summary>
        /// <param name="message">
        ///     Plaintext if authentication succeeded, all zero if authentication failed, unmodified if argument
        ///     verification fails
        /// </param>
        ...
        public static bool TryDecrypt(ArraySegment<byte> message, ArraySegment<byte> ciphertext, ArraySegment<byte> key,
            ArraySegment<byte> nonce)
        {
            if (key.Count != KeySizeInBytes)
            {
            ...
            return DecryptInternal(message.Array, message.Offset, ciphertext.Array, ciphertext.Offset, ciphertext.Count,
                key.Array, key.Offset, nonce.Array, nonce.Offset);
        }
